...
Using the `libc.so` recovered earlier, and based on the challenge context, I suspected it was a custom or modified version. After spending hours analyzing the disassembly, I turned my attention to commonly used functions—specifically `_GI___libc_write`. There, I noticed additional instructions that appeared to implement a custom encryption routine.

By carefully reversing these operations, I managed to extract both the embedded flag and the IP address the binary connects to. I automated the decryption process with the below script
